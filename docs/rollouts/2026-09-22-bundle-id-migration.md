...
- `ios/UsageMonitor/UsageMonitorKit/Sources/LocalDataPlane/LocalAppModel.swift`: `LocalAppModel.bootstrap()` calls `LocalDataMigration.migrateIfNeeded()` BEFORE `store.open()` so the new app opens the freshly-copied file.  Doc comment explaining the dependency on the transition-window App Group capability.
...
6. **Local App Group + SQLite migration (post-review concern).**  The prior state had Local reading from `group.services.jays.usage.local.monitor` and writing to `~/Library/Containers/Group/<group-hash>/Library/Application Support/LocalUsageMonitor/local.sqlite`.  After this rename, the renamed app is a different iOS application identity, so the new bundle sandbox has an empty `local.sqlite` and the new Keychain access group does not reach the old Keychain entries.  Two follow-up paths:
    - **SQLite file:**  `ios/UsageMonitor/UsageMonitorKit/Sources/LocalStore/LocalDataMigration.swift` adds a first-launch helper that looks up the legacy App Group container, copies the existing `local.sqlite` to the new bundle sandbox's Application Support path, and writes `migration_b82_2026_09_22: applied` to `UserDefaults.standard` so the migration runs at most once.  Requires the owner to keep `group.services.jays.usage.local.monitor` as a granted App Group capability on the new bundle ID during the transition window (Apple Developer Portal action — see §1).  After the window closes (no sooner than the TestFlight cutover + one release cycle), the OLD App Group capability can be removed.
