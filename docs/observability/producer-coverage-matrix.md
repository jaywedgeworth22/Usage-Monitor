...
| `claude-code` (fleet collector Claude section) | UM `scripts/fleet-usage-collector.mjs` (Claude Code block at line ~131) | v2 batch | usage, cost | anthropic | claude-code | estimated | estimated | input/output/cacheRead/cacheCreation | model | n/a | Opt-out via `USAGE_MONITOR_FLEET_DISABLE_CLAUDE=1` (audit follow-up, 2026-09-21): when set, the fleet collector's Claude section is skipped entirely so the local v2-batch stream does not double-count against native Claude OTLP. Default off (backward compat) — operators who also run native Claude OTLP must set this env. |
