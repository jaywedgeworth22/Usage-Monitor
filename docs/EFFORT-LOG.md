...
## In Progress
- **Gemini Google Cloud Console key cutover and historical AI Studio spend reconciliation (CODEX root, owner-directed 2026-07-14) — HOSTED P2 FIX FOCUSED GATE GREEN / READY TO REPUBLISH.** Isolated branch `codex-gemini-cloud-keys` is rebased on `origin/main` `54d04dd`. Both current Cloud Console authorization keys passed the non-inference model-list check without exposing key material, and the current provider rows are mapped to the exact Google projects. The implementation keeps key validation and Cloud Billing independent, fingerprints their identities server-side, quarantines stale cost/quota rows after configuration or key changes, treats an empty/provisioning export as pending rather than `$0`, retries partial poll failures even when newer pushed snapshots exist, and presents truthful key/billing/freshness/action states. Owner-supplied old-project evidence remains outside this public repository and outside totals until its billing period can be reconciled. Exact code commit `8b39ec5` passed Node 24 `npm run verify`: ESLint, TypeScript, 83 files / 590 tests, four migration-safety scenarios, SQLite backup, startup configuration, and production build. Two independent post-rebase reviews returned LAND with no P0-P2. PR #214's hosted P2 is fixed: removing Gemini billing configuration now quarantines prior fingerprinted cost from provider spend, summary totals, freshness, and budget alerts. Focused Node 24 verification passed 29 tests, changed-file ESLint, and full TypeScript. Hosted checks, merge, auto-deploy, and live fetch verification remain; no rate-limit creation, billing-account linkage, destructive provider cleanup, or historical spend write occurred.
- **Anthropic individual-account billing boundary (CODEX root, owner correction 2026-07-14) — FULL GATE + INDEPENDENT REVIEW GREEN / READY TO PUBLISH.** Isolated branch `codex-anthropic-individual-account` in `/Users/jay/apps/api-usage-monitor-anthropic-individual-account`, based on fetched `origin/main` `2cf8ab0`. Official Anthropic documentation confirms that individual accounts cannot use the Admin or Usage & Cost APIs. This lane removes the misleading standard-key fallback, makes individual polling a supported skip rather than a failure, preserves organization Admin-key polling, removes the unused standard-key prompt, and routes individual API activity to explicitly incomplete pushed telemetry plus Console reconciliation while Claude plan charges remain Subscription/receipt records. UI coverage, manual-fetch availability, snapshot alerts, and legacy Admin-key capability use a server-derived non-secret state rather than masked-key inference. Exact-tree Node 24 `npm run verify` passes: ESLint, TypeScript, 79 files / 493 tests, safe migration, SQLite backup, startup configuration, and production build. Independent adversarial review returned LAND with no P0-P2. Supplemental review returned HOLD on the separate alert-persistence commit `594054f`, so that unreleasable lane is no longer a prerequisite and must later rebase onto this change while preserving its snapshot-capability semantics. Hosted checks, merge, and live auto-deploy proof remain. No provider, subscription, credential, mailbox, Render-config, or production-data mutation.
- **Alert persistence config-generation corrective (CODEX builder, delegated 2026-07-14) — LOCAL + REVIEW + CANONICAL FULL GATE GREEN / PUBLICATION IN PROGRESS.** Exact reviewed implementation is committed through `930cbc37756b6c9a522f1b1d8dbe49feb282d644` on unchanged fetched `origin/main` `0420eb0c73e3b35eafbe33301df2dee2770ff8ae`. Fresh alert review and final SQLite startup-test re-review returned LAND with no P0-P3. The corrective preserves current-main `providerPollSnapshotExpected`, atomically generations every alert-affecting configuration/capability mutation, and fences evidence, incident, parent, child, summary, close, and monotonic clocks across external boundaries. The test-only full-gate repair disconnects its fixture Prisma client before child startup DDL and preserves bounded sanitized diagnostics without application behavior drift. Serialized Node `v24.18.0` canonical `npm run verify` is fully green: ESLint, TypeScript, 81 files / 534 tests, all four safe-migration scenarios, SQLite backup, startup configuration, and Next.js 16.2.10 production build. Push/PR, hosted checks, squash merge, and exact Render production SHA/health proof are next. Scheduler and OTLP stay disabled; no provider call, secret read, or production-data mutation occurred.
...
- **Anthropic cash-spend correction and historical Claude telemetry reconciliation (CODEX root, owner-directed 2026-07-14) — READY TO LAND / FULL GATE + INDEPENDENT REVIEW GREEN.** Isolated implementation `/Users/jay/apps/api-usage-monitor-anthropic-spend-correction` on `codex-anthropic-cash-spend-correction` (hyphenated because legacy local branch `refs/heads/codex` blocks the requested `codex/*` namespace) makes exact Claude Code OTLP cost a non-authoritative API-equivalent estimate across raw events and rollups, while provider/budget/project cash totals retain authoritative API and subscription charges. The dashboard keeps the estimate visible but labels it excluded from cash spend and directs users to Anthropic Console billing. The user's API keys belong to one account; the second live Provider row has only 248 all-null snapshots and no plan, allocation, subscription, rollup, alert, or external-billing dependency, but no production deletion occurred in this lane. The user-asserted two-$200-plan scenario exists only as a regression fixture: current receipt evidence does not prove two live $200 subscriptions, so no production subscription, price, or renewal anchor was created or changed. Node 24 `npm run verify` passes: ESLint, TypeScript, 77 files / 476 tests, safe migration, SQLite backup, startup configuration, and production build. Independent adversarial review returned LAND with no P0-P2 after the billing-copy correction. No production mutation, commit, push, PR, merge, deploy, scheduler enable, or OTLP enable in this sublane.
- **Alert-notification persistence isolation (CODEX root, production incident follow-up 2026-07-14) — PR #204 MERGED/DEPLOYED PREMATURELY; SUPERSEDED BY ISOLATED CONFIG-GENERATION CORRECTIVE ABOVE.** PR #204 merged as `56d532ec` before root re-review clearance. The idle `codex-alert-persistence-corrective` source tree addressed the first persistence/operation/evidence blockers but still failed disable/re-enable ordering for unchanged snapshot and no-snapshot evidence. Its relevant implementation has been transferred to the current-main Codex branch above for the reviewed generation-based correction; do not publish the idle source worktree independently.
- **Generic ingest replay response accounting (CODEX root, production-smoke follow-up 2026-07-14) — READY TO LAND / FULL LOCAL GATE + INDEPENDENT REVIEW GREEN.** Exact-live recovery smoke proved SQLite idempotency but exposed that the response counted every active input as persisted, so an already-existing byte-identical replay reported `accepted: 1` although no row was inserted. Isolated branch `codex-ingest-accepted-count` at `origin/main` `bd838f7` now defines `persisted` as newly inserted rows only and adds direct persister plus route regressions covering first insert, identical replay, mixed new/existing inputs, and retention tombstones. Node 24 `npm run verify` passes: ESLint, TypeScript, 76 files / 462 tests, safe migration, SQLite backup, startup configuration, and production build. Independent read-only review found the semantics and coverage correct with no P0-P2; its non-blocking route-assertion strengthening was applied and focused tests, scoped ESLint, TypeScript, and diff checks re-passed. Ready PR, hosted gates, and exact-live deploy remain; no Render config, provider-money, or additional production-smoke writes are in scope.
- **OTLP retry-storm isolation and shared ingest admission (CODEX operator, owner-directed 2026-07-14) — READY TO LAND / FULL GATE + INDEPENDENT REVIEW GREEN.** Render metrics on exact main `6ae30eb` with the boot scheduler disabled and readiness DB probe skipped showed continued Prisma P1008 plus an inbound retry storm (up to 2,036 HTTP 502 responses in one minute, 485 HTTP 429 responses in another, and sustained HTTP 499 client cancellations). Branch `codex-otlp-ingest-admission` at current main `f3f5f1f` now has a default-on `OTLP_METRICS_INGEST_ENABLED` emergency switch: admitted authenticated requests receive `503` and rate-limited overflow receives `429`, both with `Retry-After: 300` before decoding or database access. A process-global one-writer admission guard is shared by generic and OTLP SQLite writes (`503` plus `Retry-After: 5` on overlap). Full Node 24 `npm run verify` is green: lint, TypeScript, 76 files / 460 tests, migration safety including Litestream external-state preservation, SQLite backup, startup configuration, and production build. Independent hostile review returned LAND with no P0-P2. Hosted gates, exact-live activation with the OTLP flag set false, sustained no-P1008 proof, and one authenticated generic ingest plus identical replay proof remain required; no smoke writes before isolation is live.

- **Render DB/Litestream state recovery (CODEX + upstream/local auditors, owner-directed 2026-07-14) — IMPLEMENTED LOCALLY / READY TO LAND.** Isolated branch `codex-litestream-state-recovery` in `/Users/jay/apps/api-usage-monitor-litestream-state-recovery`, rebased onto `origin/main` `6ae30eb`. Exact production startup evidence shows Prisma's Litestream-only `--accept-data-loss` fallback dropped the nonempty `_litestream_seq` row and forced a fresh 60,858,513-byte LTX upload before five-second P1008 failures resumed. The fix declares `_litestream_seq` and `_litestream_lock` as Prisma external tables, removes the destructive fallback, and verifies arbitrary schemas plus opaque rows survive additive schema synchronization while destructive application changes still fail closed. Post-rebase Node 24 full `npm run verify` passed (74 files / 452 tests plus lint, TypeScript, migration safety, SQLite backup, startup config, and production build); independent review returned LAND with no P0-P2, and the review nits passed focused re-verification. This lane does not touch the separate scheduler gate or Infisical provider-money files. Hosted gates, exact-revision deploy, authenticated ingest/replay, and sustained runtime receipts remain required.
- **Render emergency boot-scheduler gate (CODEX operator, owner-directed 2026-07-14) — PR #192 MERGED.** PR #192 merged as `6ae30eb` after full local and hosted gates. The default-on `USAGE_SCHEDULER_ENABLED` switch permits an operator to isolate boot polling temporarily while preserving authenticated usage ingest; exact-revision deploy, health plus ingest/replay proof, and explicit re-enable criteria remain required.
- **Render `/api/ready` liveness failsafe (CODEX, emergency owner-directed 2026-07-14) — PRS #181/#188 MERGED / TEMPORARY BRIDGE ACTIVE.** PR #181 merged as `938af7f`; PR #188 merged as `27cf61a` after complete local and hosted gates. Render reports `healthCheckPath=/api/health`, and `RENDER_READINESS_HTTP_COMPATIBILITY=true` makes `/api/ready` skip its uncancellable Prisma probe. That bridge prevents the readiness route itself from starting another poison query, but exact live instances still resumed P1008 after boot, so database recovery and ingest proof remain open.
- **Render SQLite readiness/restart-loop repair (CODEX, PR #178 merged as `d03b1b8`, branch `codex-render-sqlite-readiness`, 2026-07-14) — MERGED / FOLLOW-UP REQUIRED.** Render's paid Starter service is `not_suspended`; this was not a plan/usage-limit failure. The repair made scheduled full `VACUUM` explicit-opt-in, separated `/api/health` liveness from strict diagnostics, and bounded readiness behavior. Node 24 full verification, independent review, and hosted gates passed; the Litestream state-preservation and DB-backed recovery follow-up above is now active.
