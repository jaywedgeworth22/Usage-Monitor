...
## In Progress
- **Render `/api/ready` liveness failsafe (CODEX, emergency owner-directed 2026-07-14) — IN PROGRESS / REVIEW LAND.** Branch `codex-render-readiness-failsafe`, isolated worktree `/Users/jay/apps/api-usage-monitor-render-readiness-failsafe`, based on current fetched `origin/main`. `/api/ready` keeps strict JSON diagnostics while always returning HTTP 200, failed SQLite probes are cached for 60 seconds, and hung probes stay coalesced so Render's stale five-second poll cannot restart or hammer the sole SQLite process. Route suite 11/11, ESLint, TypeScript, and diff-check pass; independent hostile review returned LAND with no P0-P2 findings. PR gates, merge, auto-deploy, and live revision/uptime verification remain. This owner-directed failsafe supersedes the narrower compatibility lane for recovery semantics; no Infisical feature files are in scope.
- **Render `/api/ready` liveness compatibility hotfix (CODEX, PR #180 merged as `40afc02`, 2026-07-14) — MERGED / SUPERSEDED BY PR #181.** The narrower bridge made database-only readiness failures HTTP 200 behind `RENDER_READINESS_HTTP_COMPATIBILITY=true` while leaving scheduler, backup, and startup failures at HTTP 503. Full Node 24 verify and independent review passed before merge. Live logs still showed five-second Prisma P1008 probes and a kill after the five-minute grace, so owner-directed PR #181 above replaces the flag-gated behavior with liveness-safe transport for every diagnostic state plus a 60-second failed-probe backoff.
- **Render SQLite readiness/restart-loop repair (CODEX, PR #178 merged as `d03b1b8`, branch `codex-render-sqlite-readiness`, 2026-07-14) — MERGED / FOLLOW-UP REQUIRED.** Render's paid Starter service is `not_suspended`; this was not a plan/usage-limit failure. Root cause at deployed `cdee64b4`: boot-time retention ran a full exclusive `VACUUM` against the ~129.7 MB SQLite file, strict `/api/ready` probes piled up/timed out (`Prisma P1008`), and Render killed/restarted the sole instance into the same maintenance cycle, producing intermittent `x-render-routing: dynamic-paid-error` 502s. The repair makes scheduled VACUUM explicit-opt-in, uses database-independent `/api/health` in declared Render config, coalesces uncancellable SQLite readiness probes, and adds a bounded database-only cold-start grace that never re-arms after first success. Independent hostile review found no P0/P1/P2; Node 24 full `npm run verify` passed (73 files / 443 tests plus lint, TypeScript, migration, backup, startup-config, and build), and GitHub CI/CodeQL/gitleaks passed. Render did not synchronize the declared health-check path, so PR #181 above is the active production-recovery lane. Emergency env overrides remain temporary; no plan or billing change occurred.
