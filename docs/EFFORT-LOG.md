...
- **Alert-notification P1008 isolation (CODEX root, production incident follow-up 2026-07-14) — READY TO LAND / FULL GATE + INDEPENDENT REVIEW GREEN.** Exact-main `2d50264` aborts the whole scheduler result when final `ProviderAlertNotification` bookkeeping hits P1008 after provider polling and prior money-path maintenance already committed. This separate lane defers only that exact transient model/code pair, returns a structured result, logs once, and retries on the next cycle. Programming, schema, channel-state, subscription, renewal, and retention failures remain fatal. Integration coverage proves successful channel state is persisted before the summary timeout and suppresses a next-cycle duplicate. Node 24 full verify passed (77 files / 472 tests plus all safety/build checks); hostile re-review returned LAND with no P0-P2. This does not replace shared writer serialization and does not hold SQLite admission across external alert sends.
