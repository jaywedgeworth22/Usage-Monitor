...
## Deployed
- **Isolated Socratic primary-account Infisical reader (CODEX root, owner-directed 2026-07-15) — MERGED / DEPLOYED / LIVE / DEFAULT-OFF.** PR #286 merged as `4e84fe39d4d86bc96c1e0bac1676be2592cc18d8`; Render deploy `dep-d9bp29e7r5hc73ekne40` is live and ready on that exact revision. The fixed-path reader remains default-off and unconfigured. No identity, API-key, Infisical value, provider, database, or PagerDuty mutation occurred during publication.
...
## In Progress
- **Socratic primary-bridge literal-byte reader correction (CODEX, root-delegated 2026-07-15) — PR #293 OPEN / HOSTED DATE-FIX REMEDIATION GREEN LOCALLY.** Branch `codex/st-primary-unexpanded-reader` / worktree `/Users/jay/apps/api-usage-monitor-st-primary-unexpanded-reader` changes only fixed ST-primary bridge manifest/member reads to request unexpanded Infisical bytes before SHA-256 verification; ordinary ST/CT/shared and ST-bootstrap exact reads remain reference-expanded. Commit `322ea8e` passed independent LAND and the full Node 24 gate (89 files / 830 tests). Hosted `verify` then exposed three clock-dependent subscription-fixture failures because the runner date made formerly future renewal dates invalid; the bridge tests passed. A Date-only fake clock now stabilizes that existing fixture without changing product logic or faking timers; affected tests pass 51/51 with scoped ESLint, TypeScript, and diff-check. Push/check/merge follow-through is in progress. No secret/env/provider/database/Render/PagerDuty mutation or manual deploy.
- **Cloudflare explicit renewal-window legacy handoff (CODEX root, 2026-07-15) — IN PROGRESS / RESERVED.** Live guarded probe on deployed `9dacab7` returned sanitized `external_billing_ineligible` without mutating the canonical Subscription. Authenticated sanitized provider state proves the authoritative canonical Workers Paid record is fresh, paid, exact `$5.00` USD monthly, but Cloudflare reports `2026-06-16T19:02:36Z` through its midnight renewal boundary `2026-07-16T00:00:00Z`, so the general exact-calendar-period adoption gate correctly excludes it. Scope: add a Cloudflare-only, handoff-only explicit-window candidate path with tight duration/status/source/identity/freshness/currency/cents/cadence guards; keep general automatic adoption strict; prove no broader provider or PagerDuty behavior. Isolated branch `codex/cloudflare-explicit-renewal-handoff`, worktree `/Users/jay/apps/api-usage-monitor-cloudflare-explicit-renewal-handoff`, base `origin/main` `9dacab7`. Probe flag has been cleared; flag-clear Render replacement is in progress. No production subscription mutation occurred.
  Root-delegated P1 remediation is publication-ready on a rebased local commit atop `origin/main` `c6c4c8f`, full-gate green, and exact-commit hostile-review LAND: the midnight-window fallback is computed only for the exact preserved built-in Cloudflare legacy row during reconciliation and never enters the global candidate map; another/custom provider, a non-legacy managed Cloudflare row, and a second same-shaped explicit authority fail closed. Focused tests passed 64/64; full `npm run verify` passed ESLint, TypeScript, 89 files / 843 tests, migration/Litestream safety, SQLite backup/startup checks, and production build. Root independently inspected the scope fix and regressions. No push, deploy, production, env, secret, provider, database, or PagerDuty mutation.
- **Cloudflare handoff readiness reason observability (CODEX implementation lane, root-delegated 2026-07-15) — IMPLEMENTED / INDEPENDENT LAND / FULL GATE GREEN / LOCAL COMMIT PENDING.** Isolated branch `codex/cloudflare-handoff-readiness` / worktree `/Users/jay/apps/api-usage-monitor-cloudflare-handoff-readiness`, exact base `origin/main` `5fcc697`. Every completed scheduler tick explicitly allowlists only aggregate counts, `maintenanceHealthy`, and the bounded `cloudflareLegacyHandoff` enum into runtime state; `/api/ready` exposes that existing `lastRun` summary even on the first unhealthy cycle. Storage and response-copy normalization strip injected target/env/billing/provider-error fields. A handoff-only `charge_proof_missing` cycle is independently proven unhealthy while readiness remains below the existing repeated-failure threshold (`consecutiveFailures=1`, `readinessReason=null`); alert generation and thresholds are unchanged. Focused verification passed 3 files / 38 tests; full `npm run verify` passed ESLint, TypeScript, 89 files / 829 tests, migration safety, SQLite backup/startup checks, and production build; diff-check passed. Independent re-review LAND with no P0-P2. No IDs, env values, billing payloads, provider errors, secrets, PagerDuty alerts, push, deploy, production, or environment mutation.
- **Socratic primary-account Infisical bridge reader (CODEX, root-delegated 2026-07-15) — DONE / INDEPENDENT LAND / LOCAL COMMIT.** Rebased onto current `origin/main` `79e44cb` on isolated branch `codex/st-primary-bridge-reader`. Adds an independently default-off, read-only source fixed to ST `prod` `/usage-monitor/st-primary/v1` with a strict duplicate-member-safe `BRIDGE_MANIFEST_V1` complete-set contract for exact Gemini/DeepSeek keys; atomic sequence/fingerprint/replay fencing; last-known-good retention; revoked-first keyless tombstones; separate `SocraticTrade.com · Primary account` rows; and inactive exact-credential aliases that cannot poll manually. Ordinary app-root sync cannot claim bridge rows. Browser ownership claims, managed credential/active/label edits, deletion, and unsafe key/binding previews fail closed, including malformed bindings and unreadable managed-label envelopes; unrelated billing config remains editable. Independent hostile review found no remaining P0-P2. Post-rebase focused gate passed 3 files / 84 tests; exact `npm run verify` passed lint, TypeScript, 89 files / 798 tests, migration safety, SQLite backup/startup checks, and production build. No Prisma migration, identity creation, secret read/output, live provider/Infisical/Render/PagerDuty mutation, push, PR, merge, or deploy.
- **Cloudflare legacy Subscription exact handoff (CODEX implementation lane, root-delegated 2026-07-15) — REBASED / HOSTILE LAND / FULL GATE GREEN / PR READY.** Isolated branch `codex/cloudflare-legacy-handoff` / worktree `/Users/jay/apps/api-usage-monitor-cloudflare-legacy-handoff`, exact base `origin/main` `4e84fe39`. The default-off canonical Subscription UUID path runs inside the existing SQLite writer-locked adoption transaction before reconciliation and requires the active built-in canonical Cloudflare provider, exact external identity, no positive ProviderPlan fixed fee/shape/guard collision, fresh current canonical paid-recurring authoritative exact-cent USD terms, matching local cadence/window, and the exact deterministic current-period event plus watermark. A harmless live-shaped placeholder plan with no fixed fee is allowed. Success changes only management ownership, the exact guard, and `autoRenew` on the same row; UUID, owner display name, project, terms, notes, knobs, event, and history are preserved. Already-managed replay is a no-op, while an unmanaged row with a non-null guard is permanent owner relinquishment and is never retaken. Sanitized results contain only bounded status; any configured blocked state marks scheduler maintenance unhealthy without synthesizing provider/PagerDuty alerts. Post-rebase focused verification passed 3 files / 98 tests; exact `npm run verify` passed ESLint, TypeScript, 89 files / 828 tests, migration safety, SQLite backup/startup checks, and production build. Independent hostile review LAND on the exact source commit `869ccdf` with no P0-P2; the rebase conflict was effort-log-only. No deploy, production/database/provider/env/identity/key mutation, PagerDuty, or unrelated billing work.
