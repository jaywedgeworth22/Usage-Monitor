...
## In Progress
- **PagerDuty stale/missing-source alert-noise audit and snapshot-capability fix (CODEX root + subagent, owner-directed 2026-07-15) — LOCAL PATCH VERIFIED / NOT PUSHED.** Root cause: PagerDuty delivery used full snapshot alerts while `/api/providers` replaced them with budget-only alerts whenever canonical budget rows existed; additionally `providerPollSnapshotExpected` only suppressed Anthropic individual accounts and still expected snapshots from generic/push rows plus documented no-poll built-ins (`voyage`, `fmp`, `finnhub`, `alphavantage`, `marketstack`, `tiingo`, `massive`, `fred`, `robinhood`). Fresh isolated branch `codex/pagerduty-snapshot-capability` / worktree `/Users/jay/apps/api-usage-monitor-pagerduty-snapshot-capability` on current `origin/main` `f3184d3` now shares snapshot-capability semantics across PagerDuty and provider APIs, preserves budget alerts, and suppresses impossible stale/missing-snapshot noise for blind providers. Verification under Node 24: focused Vitest 3 files / 12 tests, targeted ESLint, TypeScript, and `git diff --check` passed. No production, PagerDuty, provider, database, Render, secret, push, PR, merge, or deploy mutation yet.
- **Congress.Trade full usage-ingest + OTLP activation closeout (CODEX root, owner-directed 2026-07-15) — MERGED / LIVE GREEN / SCHEDULER + OTLP ACTIVE; PR #254 FOLLOW-UP IN HOSTED GATES.** PR #251 merged as `f3184d32718b5b8933a7641bf50baac81cebe753`; Render production `/api/health` and `/api/ready` report exact revision `f3184d3`, `ok/status=ready`, DB latency 0-1 ms, backup/startup active, and scheduler healthy. The code serializes internal scheduler/provider/retention SQLite write phases through shared admission so OTLP/generic pushed ingest cannot race scheduled writes, with FIFO/reentrant internal admission, abort-before-write, and retention read-vs-write coverage. Codex review P1 was resolved by wrapping `ensureAgentSyncProviderSeeded`, `materializeDueSubscriptions`, and `rollForwardProviderRenewals`. Follow-up PR #254 (`0b7616a`) narrows alert-maintenance admission so alert persistence writes are admitted but external Slack/Resend/PagerDuty/webhook waits do not hold the usage-ingest lease; local `npm run typecheck` passed and focused alert/maintenance tests passed 53/53, hosted checks pending. Verification for #251: local focused 4 files / 36 tests, local full `npm run verify` passed ESLint, TypeScript, 83 files / 597 tests, migrate-safe 4/4, SQLite backup, startup config, and Next build; hosted CI, CodeQL, and gitleaks passed. Authenticated empty OTLP metrics POST returned HTTP 202 `{accepted:0}`. Three readiness samples after deploy stayed healthy; scheduler last tick succeeded with consecutive failures 0. No provider-money call, production data repair, or secret output was performed.
