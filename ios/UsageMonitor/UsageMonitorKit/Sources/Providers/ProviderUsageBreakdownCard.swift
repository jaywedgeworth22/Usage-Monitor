...
/// A Provider-aware card rendering the usage / storage breakdown by application
/// for Backblaze B2 (buckets) and other multi-service providers.
public struct ProviderUsageBreakdownCard: View {
    public let providerTitle: String
    public let records: [ExternalBillingRecord]

    public init(providerTitle: String, records: [ExternalBillingRecord]) {
    ...
    private var items: [UsageBreakdownItem] {
        ...
        return sorted.enumerated().map { index, record in
            ...
            return UsageBreakdownItem(
                ...
                percentage: pct,
                status: ExternalBillingCard.semanticStatus(for: record.status ?? "active"),
                color: color
            )
        }
    }
