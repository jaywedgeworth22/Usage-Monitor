...
- **+ → Add Missing Providers** creates inactive $0 cards for every known service (private SQLite; survives app updates; no keys/fees invented).  Cards are placeholders — open one and tap **Connect Account** to paste a key.  The Active / poll toggle does nothing until a key is saved.  **Add Provider** on an existing card updates that card instead of erroring.
