...
const DEFINITIONS = [
  ...
  { name: "unusual-whales", displayName: "Unusual Whales", type: "builtin", category: "Market Data", defaultRefreshIntervalMin: 1440, helpNote: "Unusual Whales exposes no account, plan, or billing endpoint. Each poll makes one minimal /api/congress/recent-trades request (limit=1) solely to read the x-uw-daily-req-count response header, the account's cumulative request count for the current provider day (resets 8:00pm ET, not midnight). That poll consumes exactly one request against the same daily counter as any other authenticated call, so new connections default to daily sync. Request limit, USD price, and renewal date are not exposed." },
