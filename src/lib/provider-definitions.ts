...
const DEFINITIONS = [
  ...
  { name: "xai", displayName: "xAI (Grok)", type: "builtin", category: "LLM/AI", helpNote: "Enter your xAI Management API key (not an inference key) above, plus Team ID below. Optionally override the key in advanced config. Reads prepaid balance, postpaid invoice preview, billing cycle, and spending limits.", needsConfig: { fields: [{ key: "teamId", label: "Team ID", placeholder: "xAI team ID", required: true }, { key: "managementKey", label: "Management API key override (optional)", placeholder: "Leave blank to use the API Key above", type: "password", advanced: true }] } },
