...
const DEFINITIONS = [
  ...
  { name: "xai", displayName: "xAI (Grok)", type: "builtin", category: "LLM/AI", helpNote: "Requires a Management API key (not an inference key) and Team ID. Reads prepaid balance, postpaid invoice preview, billing cycle, and spending limits from xAI's billing API.", needsConfig: { fields: [{ key: "teamId", label: "Team ID", placeholder: "xAI team ID", required: true }, { key: "managementKey", label: "Management API key override (optional)", placeholder: "Leave blank to use the main API key", type: "password", advanced: true }] } },
  ...
  { name: "voyage", displayName: "Voyage AI", type: "builtin", category: "Vector DB", usesApiKey: false, helpNote: "No non-billable account or usage API is available. This row is push/manual; no key is requested or polled. Add a Subscription or push cost events so projected charts include Voyage." },
  { name: "roic", displayName: "ROIC.ai", type: "builtin", category: "Market Data", usesApiKey: false, helpNote: "No documented non-billable usage/billing API is wired here yet. This row is push/manual: track the ROIC plan as a Subscription (or push cost events) so it appears in projected costs and Overview. ROIC_API_KEY is not used for polling." },
