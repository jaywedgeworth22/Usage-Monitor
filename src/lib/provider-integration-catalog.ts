...
const CATALOG: Record<CatalogProviderName, ProviderIntegrationProfile> = {
  ...
  oracle: defineProfile({
    ...
    credentialInputs: ["Tenancy OCID, user OCID, API-key fingerprint, one supported commercial US tenancy home region (Ashburn, Chicago, Phoenix, or San Jose), and a PEM API signing private key encrypted in server-only provider secret config. Other OCI realms are rejected rather than signed with the commercial endpoint. COST queries require OCI USAGE_REPORT_READ or USAGE_REPORT_MANAGE; USAGE_ANALYSIS_READ is usage-only.", "Optional verified budget currency, quota compartment OCID, and an explicit bounded list of OCI service names."],
    ...
    limitations: ["Usage API DAILY aggregation reports complete day-aligned data but can publish cost up to 48 hours late; the exact reported amount remains visible with incomplete coverage.", "The connector currently supports only four commercial US home regions because it constructs the commercial OCI endpoint; other realms are rejected until a trusted region-to-realm endpoint map is implemented.", "Optional service detail or limit metadata is fail-open and never prunes prior detail when its source is unavailable, malformed, or over the bounded cardinality limit."],
