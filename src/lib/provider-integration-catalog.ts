...
const CATALOG: Record<CatalogProviderName, ProviderIntegrationProfile> = {
  ...
  "google-ai": defineProfile({
    ...
    reads: ["Available model count and any rate-limit headers returned by the Gemini API.", "Project-level Gemini request counts, request/token rate-quota usage, and request/token quota limits from documented Service Runtime metrics in Cloud Monitoring.", "Gemini API regular cost, credits, project, SKU, usage quantity, and report-through time from the configured standard billing export."],
    stores: ["Sanitized month-to-date project request counts and request/token quota metadata; method and credential identifiers are discarded.", "Standard billing export cost/SKU rows when that independent channel is configured."],
    ...
    limitations: ["Cloud Monitoring request_count may be delayed by up to 30 minutes, quota usage by about four minutes, and quota-limit samples are daily; an empty response is kept unknown rather than presented as zero.", "Cloud Billing export is delayed, can take days to backfill initially, and requires an exact Gemini project when the dataset contains more than one.", "A replacement key is shown as unchecked until Verify & fetch binds a fresh validation result to that credential."],
