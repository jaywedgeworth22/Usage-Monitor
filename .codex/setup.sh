#!/usr/bin/env bash
# Codex Cloud setup for Usage-Monitor. Secrets must be runtime environment variables
# when the agent phase needs them; setup-only secrets are removed before the agent runs.
set -euo pipefail
cd "$(dirname "$0")/.."

echo "==> Codex Cloud setup: Usage-Monitor"
...
echo "Slack project: ${SLACK_PROJECT:-Usage-Monitor}"
...
echo "Setup complete: Usage-Monitor"
